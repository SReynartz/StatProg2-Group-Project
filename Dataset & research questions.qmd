...
The remaining work is split into the following steps, each trackable as a separate GitHub Issue and assignable to one group member:
...
2. **Exploratory analysis: seasonality of alcohol-related accidents** (Selma) — compute monthly means and the monthly % share of alcohol-related accidents relative to total accidents; produce *Figure 1* and *Table 1*.
3. **Feature engineering for the predictive model** (Yannick) — build a linear time-trend variable and calendar-month dummies from `JAHR`/`MONAT`; split the series into a training period and a held-out test period (e.g. last 12 months).
4. **Model fitting and evaluation** (Selma) — fit the linear regression on the training period, generate predictions for the test period, and produce *Table 2* and *Figure 2* (incl. RMSE).
5. **Visualisation and report polishing** (Selma & Yannick) — finalize styling of Figures 1–2 and Table 1–2, write up interpretation text, and assemble the rendered report/website.
